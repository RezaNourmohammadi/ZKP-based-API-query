...
app.post('/multiQuery', async (req, res) => {
  ...
  try {
    ...
    let witnessFileName = "multiQueryProofEngine/temp/witness128.wtns"
    let programFileName = "multiQueryProofEngine/programs/firstLevel128.program"
    let abiFileName = "multiQueryProofEngine/abi/firstLevelAbi128.json"
    let proofFileName = "multiQueryProofEngine/temp/proof128.json"
    let verificationKeyPath = "multiQueryProofEngine/keys/verification128.key"
    let provingKeyPath = "multiQueryProofEngine/keys/proving128.key"

    let witnessExecStr = `zokrates compute-witness --verbose -s ${abiFileName} -i ${programFileName} -o ${witnessFileName} -a ${circuitArgs.toString().replaceAll(",", " ")}`

    let proveExecStr = `zokrates generate-proof --proving-scheme gm17 --backend ark -i ${programFileName} --witness ${witnessFileName} -p ${provingKeyPath} --proof-path ${proofFileName}`

    let verifyExecStr = `zokrates verify --proof-path ${proofFileName} --verification-key-path ${verificationKeyPath}`

    let witnessProcess = execShell(witnessExecStr)
    witnessProcess.once("exit", (code) => {
      let proveProcess = execShell(proveExecStr)
      proveProcess.once("exit", (code) => {
        let verifyProcess = execShell(verifyExecStr)
        verifyProcess.once("exit", (code) => {
...
app.post('/multiQuery16', async (req, res) => {
  ...
  try {
    ...
    let witnessFileName = "multiQueryProofEngine/temp/witness16.wtns"
    let programFileName = "multiQueryProofEngine/programs/firstLevel16.program"
    let proofFileName = "multiQueryProofEngine/temp/proof16.json"

    let witnessExecStr = `echo $PWD; zokrates compute-witness --verbose -s multiQueryProofEngine/abi/firstLevelAbi16.ts -i ${programFileName} -o ${witnessFileName} -a ${circuitArgs.toString().replaceAll(",", " ")}`

    let proveExecStr = `echo $PWD; zokrates generate-proof --proving-scheme gm17 --backend ark -i ${programFileName} --witness ${witnessFileName} -p multiQueryProofEngine/keys/proving16.key --proof-path ${proofFileName}`

    let verifyExecStr = `echo $PWD; zokrates verify --proof-path ${proofFileName} --verification-key-path multiQueryProofEngine/keys/verification16.key`
...
function execShell(execStr: string): ChildProcess {
  let p = exec(execStr, (error, stdout, stderr) => {
    if (error) {
...
app.post('/secondlevel16', async (req, res) => {
  ...
  try {
    ...
    if (secondZokProvider === undefined || poseidon === undefined) {
      ...
      let secondLevelProgram = fs.readFileSync(path.resolve(__dirname, "./keys/secondLevel16.program"), null)
      let program = new Uint8Array(secondLevelProgram.buffer)
      let input: CompilationArtifacts = {
        program,
        abi: proofAbi
      }
      let verificationKeyFile = fs.readFileSync(path.resolve(__dirname, "./keys/verification16.key"), null)
      ...
      let provingKeyFile = fs.readFileSync(path.resolve(__dirname, "./keys/secondProving16.key"), null)
      ...
      verificationKeyFile = fs.readFileSync(path.resolve(__dirname, "./keys/verification16.key"), null)
...
export function genProof(program: Uint8Array, witness: Uint8Array): Proof | undefined {
  ...
  let f = fs.readFileSync(path.resolve(__dirname, "./keys/proving.key"), null)
...
export function verifyProof(proof: Proof): boolean | undefined {
  if (firstZokProvider === undefined) return undefined
  let f = fs.readFileSync(path.resolve(__dirname, "./keys/verification.key"), null)
