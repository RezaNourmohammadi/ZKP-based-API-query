...
import { ZERO_HASHES_377 as ZERO_HASHES } from "../lib/poseidon377/zeroHashes";
...
export function hexToBigNumStr(args: any[]): string[] {
    let ret: string[] = []
    args.forEach( e => {
        e = e + ""
        if(e.startsWith("0x")){
            ...
            try{
                let numStr = getBigInt(e).toString()
                ret.push(numStr)
            } catch {
                ret.push(BigInt(hexlify(e)).toString())
            }
        }
    })
    return ret
